...
include("saberpairs.jl")
export SABER_PAIRS, SaberFeed

mutable struct SaberFeed <: DataFeed
...
function validatepairs(d::SaberFeed; mkts::Vector{<:AbstractString}=d.markets)::Vector{<:AbstractString}
...
function getstate(d::SaberFeed)::JSON3.Array
    r = HTTP.get("$(d.http_server)/getState")
    if r.status == 200
        data = r.body |> String |> JSON3.read |> JSON3.read
        return data
...
function feed(d::SaberFeed, args...; kwargs...)
    length(d.markets) > 0 || raise(ArgumentError("No markets provided for Saber"))
    kwargs = Dict(kwargs)
            ...
            WebSockets.open(d.ws_server) do ws
                for data in ws
                    ...
                    !any(==(nothing), [data.bidSize, data.askSize, data.bidPrice, data.askPrice]) && begin
                        @debug "Received Saber quote " data
                        foldr(∘, [args..., extractpacket])(data)
                    end
                end
            end
        catch e
            @warn "Error with Saber feed " e
        end
    end
end
