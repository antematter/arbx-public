...
const SortableItem = sortableElement(({ value, children }) => <li>{children}</li>);
...
const Layout = ({ data, currency, sortUserHoldings, match, location }) => {
  ...
  const balance = data
    ? Math.round(data.map((coin) => coin.PRICE * coin._amount).reduce((a, b) => a + b, 0) * 100) /
      100
    : null;
      ...
      <Top>
        <div>
          ...
          <section>
            ...
            <div>
              <TotalBalance>{balance !== null && formatCurrency(balance, currency)}</TotalBalance>
              <div>
                <Toolbar>
                  <LinkButton
                    fullWidth
                  ...
                  <MainRow>
                    ...
                    <RightCol>
                      <Balance>
                        ...
                        <data value={coin._amount}>{`${coin._amount} ${coin.symbol}`}</data>
                      </Balance>
                      <LinkButton size="md" square variant="menu" to={`wallet/${coin.symbol}`}>
                        <MenuIcon />
                      </LinkButton>
                    </RightCol>
                  </MainRow>
