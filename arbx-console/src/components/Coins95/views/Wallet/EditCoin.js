...
const Layout = ({ coin, currency, holdings, history, setUserHoldings, deleteUserHoldings }) => {
  ...
  const goBack = () => history.push(`/market-watch/wallet`);
  const handleAccept = () => {
      ...
      <EditWindow onClick={(e) => e.stopPropagation()}>
        ...
        <WindowContent>
          ...
          <Toolbar style={{ justifyContent: 'flex-end' }}>
            ...
            <Button style={{ width: '50%' }} onClick={handleAccept} disabled={!data} primary>
              OK
            </Button>
          </Toolbar>
        </WindowContent>
...
const mapStateToProps = (state, ownProps) => {
  ...
  const holdings = state.user.wallet[coin] ? state.user.wallet[coin].amount : null;
  return {
    coin,
    holdings,
...
const mapDispatchToProps = (dispatch) => ({
  setUserHoldings: ({ amount, coin }) => dispatch(setUserHoldings({ amount, coin })),
