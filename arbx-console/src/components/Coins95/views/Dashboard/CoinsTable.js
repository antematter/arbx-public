...
import { TableHead, TableBody, TableRow, TableHeadCell, TableDataCell } from 'react95';
  ...
  render() {
    ...
    if (!data) {
      ...
      tableData = orderedData.map((coinData, i) => {
        const { name, coinName, symbol, imageURL, PRICE = 0, CHANGEPCT24HOUR = 0 } = coinData;
        return (
          <TableRow key={i} onClick={() => history.push(`coins/${symbol}`)}>
            ...
            <TableDataCell style={{ textAlign: 'right' }}>{PRICE.toFixed(2)}</TableDataCell>
      ...
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableHeadCell onClick={() => this.handleChangeOrder('price')}>Price</TableHeadCell>
            <TableHeadCell onClick={() => this.handleChangeOrder('change')}>Change</TableHeadCell>
          </TableRow>
        </TableHead>
