...
import { TableHead, TableBody, TableRow, TableHeadCell, TableDataCell, Checkbox } from 'react95';
  ...
  render() {
    ...
    if (!data) {
      ...
      tableData = orderedData.map((coinData, i) => {
        const { name, coinName, symbol, imageURL, sortOrder, isFollowed } = coinData;
        const onClick = () =>
          history.push({
        ...
        return (
          <TableRow key={i}>
            ...
            <TableDataCell
              ...
              <SCheckbox readOnly checked={isFollowed} variant="flat" value={name} />
            </TableDataCell>
          </TableRow>
        );
      });
      ...
      <>
        <TableWrapper>
          <Table>
            <TableHead>
              <TableRow>
                <TableHeadCell onClick={() => this.handleChangeOrder('name')}>Name</TableHeadCell>
                <TableHeadCell onClick={() => this.handleChangeOrder('rank')}>Rank</TableHeadCell>
                <TableHeadCell onClick={() => this.handleChangeOrder('following')}>
        ...
        <TableFooter>
          <WellContainer>
            <Well>
              {data ? `Showing ${tableData.length} coin(s) of ${data.length} total` : 'Loading...'}
            </Well>
            <Well draggable />
          </WellContainer>
        </TableFooter>
